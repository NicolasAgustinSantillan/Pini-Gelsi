...
--**************   PROCEDMIENTOS DE CONTABLE *****************************
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_ObtenerListaPlanCuentaRubro')
DROP PROCEDURE usp_ObtenerListaPlanCuentaRubro
...
IF NOT EXISTS (SELECT IdProductoTienda FROM PRODUCTO_TIENDA)
BEGIN
	BEGIN
		DECLARE @IdTienda INT = 1
		DECLARE @Resultado BIT
		DECLARE @IdProducto INT = 1
		EXEC usp_RegistrarProductoTienda @IdProducto, @idTienda, @Resultado;
		SET @IdProducto = 2
		EXEC usp_RegistrarProductoTienda @IdProducto, @idTienda, @Resultado;
		SET @IdProducto = 3
		EXEC usp_RegistrarProductoTienda @IdProducto, @idTienda, @Resultado;
...
--PROCEDMIENTO PARA CONTABLE
create procedure usp_ObtenerListaPlanCuentaRubro
as
begin
...
begin
	...
	IF NOT EXISTS (SELECT * FROM CUENTA WHERE rtrim(ltrim(Tipo)) = rtrim(ltrim(@Tipo)))

		INSERT INTO CUENTA(Numero, RubroId, Tipo) 
		VALUES(@PlanCuenta, @Rubro, @Tipo)
